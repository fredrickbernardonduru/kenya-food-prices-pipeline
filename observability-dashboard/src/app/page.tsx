...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className="flex items-center gap-1.5">
      <span className={`inline-block w-2 h-2 rounded-full ${dot}`} style={{ background: color }} />
      <span className="text-xs font-mono uppercase tracking-widest" style={{ color }}>{status}</span>
    </span>
  );
}
...
function MetricCard({ label, value, sub, accent, delay = 0 }: {
  ...
  return (
    <div className="card-hover relative bg-surface border border-border rounded-lg p-4 overflow-hidden" style={{ animationDelay: `${delay}ms` }}>
      <div className="absolute top-0 right-0 w-16 h-16 opacity-10 rounded-bl-full" style={{ background: accent || "#00E5A0" }} />
      <p className="text-sub text-xs font-mono uppercase tracking-widest mb-2">{label}</p>
      <p className="count-anim text-2xl font-bold font-mono" style={{ color: accent || "#E8EDF3" }}>{value}</p>
...
function SectionHeader({ title, tag }: { title: string; tag?: string }) {
  return (
    <div className="flex items-center gap-3 mb-4">
      ...
      {tag && <span className="ml-auto text-xs font-mono text-dim border border-border px-2 py-0.5 rounded">{tag}</span>}
    </div>
  );
}
      ...
      <header className="sticky top-0 z-20 border-b border-border bg-bg/80 backdrop-blur-md">
        <div className="max-w-screen-xl mx-auto px-6 h-14 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="font-mono text-sm font-semibold text-text tracking-tight">KENYA PIPELINE OBSERVATORY</span>
          ...
          <div className="flex items-center gap-6">
            ...
            <button onClick={fetchAll} className="text-xs font-mono text-sub hover:text-green transition-colors border border-border hover:border-green/40 px-3 py-1.5 rounded">
              ↻ Refresh
            </button>
            <span className="text-xs font-mono text-dim">{timeStr}</span>
          </div>
        </div>
      </header>
      ...
      {loading ? (
        <div className="flex items-center justify-center h-96">
          <div className="text-center">
            <div className="text-green font-mono text-lg mb-2">Initializing<span className="cursor">_</span></div>
        ...
        <main className="max-w-screen-xl mx-auto px-6 py-8 space-y-8">
              ...
              <div className="card-hover bg-surface border border-border rounded-lg p-5">
                ...
                {health?.services.database.latency !== undefined && (
                  <div className="mt-3 pt-3 border-t border-border flex items-center gap-2">
                    ...
                    <span className="text-xs font-mono text-green ml-auto">{health.services.database.latency}ms</span>
                  </div>
                )}
              </div>
                ...
                {health?.services.airflow.lastRun && (
                  <div className="space-y-1.5">
                    ...
                    <div className="flex justify-between text-xs font-mono">
                      ...
                      <span className="text-text">{new Date(health.services.airflow.lastRun.start_date).toLocaleDateString()}</span>
                    </div>
                  </div>
                )}
                <div className="mt-3 pt-3 border-t border-border flex items-center justify-between text-xs font-mono">
                  <span className="text-dim">ui</span>
                  <a href="http://localhost:8081" target="_blank" rel="noopener noreferrer" className="text-green hover:underline">localhost:8081 ↗</a>
                </div>
              </div>
                ...
                <div className="space-y-1.5">
                  <div className="flex justify-between text-xs font-mono">
                    ...
                    <span className="text-text">
                      {health?.services.freshness.last_date ? new Date(health.services.freshness.last_date).toLocaleDateString() : "—"}
                    </span>
                  </div>
                  <div className="flex justify-between text-xs font-mono">
                    <span className="text-dim">data age</span>
                    <span style={{ color: STATUS_COLOR[health?.services.freshness.status || "unknown"] }}>
                      {health?.services.freshness.days_old != null ? `${health.services.freshness.days_old} days` : "—"}
                    </span>
                  </div>
                </div>
          ...
          <section>
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
              <MetricCard label="Total Runs"   value={stats?.total_count   || "—"} accent="#3B82F6" delay={0} />
              <MetricCard label="Successful"   value={stats?.success_count || "—"} accent="#00E5A0" delay={60} />
              <MetricCard label="Failed"       value={stats?.failed_count  || "—"} accent="#FF4560" delay={120} />
              <MetricCard label="Success Rate" value={`${successRate}%`} accent={successRate >= 80 ? "#00E5A0" : "#FFB547"} sub={`${stats?.total_count || 0} total runs`} delay={180} />
              <MetricCard label="Avg Duration" value={stats?.avg_duration ? `${Math.round(parseFloat(stats.avg_duration))}s` : "—"} accent="#A855F7" sub="per successful run" delay={240} />
            </div>
          </section>
          ...
          <section>
            ...
            <div className="grid grid-cols-2 md:grid-cols-6 gap-3">
              {metrics?.rowCounts && Object.entries(metrics.rowCounts).map(([table, count], i) => (
                <div key={table} className="card-hover bg-surface border border-border rounded-lg p-3 text-center">
                  <p className="text-xl font-bold font-mono text-text">{parseInt(count as string).toLocaleString()}</p>
                  <p className="text-dim text-xs font-mono mt-1 truncate">{table}</p>
                  <div className="w-full h-0.5 mt-2 rounded-full" style={{ background: ["#00E5A0","#3B82F6","#A855F7","#FFB547","#FF4560","#00E5A0"][i % 6], opacity: 0.6 }} />
                </div>
              ))}
            </div>
          </section>
                ...
                <BarChart data={commodityData} margin={{ top: 4, right: 4, bottom: 0, left: -20 }}>
                  ...
                  <Bar dataKey="value" radius={[3, 3, 0, 0]}>
                    {commodityData.map((_, i) => (
                      <Cell key={i} fill={["#00E5A0","#3B82F6","#A855F7","#FFB547","#FF4560","#00E5A0","#3B82F6","#A855F7"][i % 8]} opacity={0.85} />
                    ))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
              ...
              <table className="w-full text-xs font-mono">
                ...
                <tbody>
                  {runs.slice(0, 8).map((run, i) => (
                    <tr key={run.run_id} className="border-b border-border/50 hover:bg-muted/30 transition-colors" style={{ animationDelay: `${i * 40}ms` }}>
                      <td className="px-4 py-3 text-sub truncate max-w-xs">{run.run_id.slice(-30)}</td>
                      <td className="px-4 py-3">
                        <span className="flex items-center gap-1.5" style={{ color: STATUS_COLOR[run.state] || "#7A8FA3" }}>
                          <span className={`w-1.5 h-1.5 rounded-full ${STATE_DOT[run.state] || "bg-dim"}`} />
                          {run.state}
                        </span>
                      </td>
                      <td className="px-4 py-3 text-sub">{run.start_date ? new Date(run.start_date).toLocaleString() : "—"}</td>
                      <td className="px-4 py-3 text-text">{run.duration_seconds ? `${run.duration_seconds}s` : "—"}</td>
                      <td className="px-4 py-3 text-dim">{run.run_type || "manual"}</td>
                    </tr>
                  ))}
                  {runs.length === 0 && (
                    <tr>
                      <td colSpan={5} className="px-4 py-8 text-center text-dim">No runs found. Trigger the DAG in Airflow first.</td>
                    </tr>
                  )}
                </tbody>
              </table>
          ...
          {/* Task Breakdown + Log Stream */}
          <section className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-surface border border-border rounded-lg p-5">
              <SectionHeader title="Task Breakdown" tag="last 20" />
              <div className="space-y-2 max-h-64 overflow-y-auto pr-1">
                {tasks.length === 0 && <p className="text-dim text-xs font-mono py-4 text-center">No task data yet.</p>}
                {tasks.slice(0, 20).map((task, i) => (
                  <div key={i} className="flex items-center gap-3 py-2 border-b border-border/40">
                    <div className={`w-2 h-2 rounded-full flex-shrink-0 ${STATE_DOT[task.state] || "bg-dim"}`} />
                    <span className="text-xs font-mono text-text w-36 truncate">{task.task_id}</span>
                    <span className="text-xs font-mono ml-auto" style={{ color: STATUS_COLOR[task.state] || "#7A8FA3" }}>{task.state}</span>
                    <span className="text-xs font-mono text-dim w-12 text-right">{task.duration_seconds ? `${task.duration_seconds}s` : "—"}</span>
                  </div>
                ))}
              </div>
            </div>
            <div className="bg-surface border border-border rounded-lg p-5">
              ...
              <div className="space-y-1.5 max-h-64 overflow-y-auto font-mono text-xs pr-1">
                {logList.length === 0 && <p className="text-dim py-4 text-center">No logs yet.</p>}
                {logList.slice(0, 20).map((log, i) => (
                  <div key={i} className="flex items-start gap-2 py-1">
                    <span className="text-dim flex-shrink-0">{log.timestamp ? new Date(log.timestamp).toLocaleTimeString() : "—"}</span>
                    <span className="flex-shrink-0" style={{ color: STATUS_COLOR[log.state] || "#7A8FA3" }}>
                      {log.state === "success" ? "✓" : log.state === "failed" ? "✗" : "▶"}
                    </span>
          ...
          <footer className="border-t border-border pt-6 pb-4 flex items-center justify-between text-xs font-mono text-dim">
            ...
            <span>Auto-refresh: 30s · Last: {timeStr}</span>
            <span>Airflow :8081 · Metabase :3000 · PostgreSQL :5432</span>
          </footer>
        </main>
      )}
    </div>
